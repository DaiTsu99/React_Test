...
const WeatherDataDisplay = ({weatherData,language}:{weatherData:any, language:any}) => {
    const momentFormatDay = () => {
        if (language == "en"){
            ...
            const dayText = mDay.format('dddd');
            return dayText;
        } else if (language == "ja"){
            const mDay2 = moment().locale('ja');
            const dayText2 = mDay2.format('dddd');
            // console.log(dayText2);
            return dayText2;
        }
    }

    const momentFormatDate = () => {
        if (language == "en"){
            const mDate = moment().locale('en');
            const dateText = mDate.format('LL');
            return dateText;
        } else if (language == "ja"){
            const mDate2 = moment().locale('ja');
            const dateText2 = mDate2.format('LL');
            // console.log(dateText2);
            return dateText2;
        }
    }
        ...
        <div className="w-3/4 h-fit bg-green-800 drop-shadow-md rounded-xl p-2">
            ...
            <div className="flex flex-row justify-between items-center">
                <div className="font-extralight text-xl text-white flex flex-row">
                    {language=="en" ? "Temperature" : (language=="ja"? "気温":"")}
                    <div className="ml-4 font-semibold text-2xl">{weatherData.main.temp}</div>
            ...
            <div className="flex flex-row justify-between mt-4">
                <div className="font-extralight text-xl text-white flex flex-row items-end">
                    <div className="flex flex-col justify-center">
                        ...
                        {language=="en" ? "Sunrise" : (language=="ja"? "日の出":"")}
                    </div>
                ...
                <div className="font-extralight text-xl text-white flex flex-row items-end">
                    ...
                    <div className="flex flex-col justify-center">
                        ...
                        {language=="en" ? "Sunset" : (language=="ja"? "日没":"")}
                    </div>
